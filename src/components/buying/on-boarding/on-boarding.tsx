...
const contents = [
	...
	<>
		<h4 className={styles.title}>בוחרים את המטבע שרוצים לרכוש ואת מס’ התשלומים</h4>
		<div className={styles.content}>
			<ChooseCurrency countPayments={2} totalPayment={1850} totalPayable={1755} />
		</div>
	</>,
	<>
		...
		<div className={styles.content}>
			<Input value="" setValue={() => null} label="מספר זהות" placeholder="123456789" type="identityNumber" />
			<Input value="" setValue={() => null} label="נייד" placeholder="052-4567890" type="phoneNumber" />
		</div>
	</>,
	<>
		...
		<div className={styles.content}>
			<ChooseCurrency countPayments={4} totalPayment={1755} totalPayable={3654} type="confirm" />
		</div>
	</>,
]
			...
			footer={
				<div className={styles.footer}>
					<div className={styles.buttons}>
						...
						<button className={styles.button} onClick={() => setStep((prev) => (prev + 1) % 4)}>
							הבא
						</button>
					</div>
					<div className={styles.steps}>
						{Array.from({ length: contents.length }, (v, idx) => (
							<div key={idx} className={`${styles.step} ${step === idx ? styles.current : ""}`} />
						))}
					</div>
				</div>
			}
